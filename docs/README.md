...
- `components/Pricing.tsx` — `PlansTable`: the three plans (Pay per use, Pro €20/month, Team €25/member/month), everything priced in credits. Used only by the UNLISTED `/pricing` page: there is no pricing section on the home page and nothing links to `/pricing` (decision 2026-09-21). Source of truth for the numbers: domain-expansion `areas/product/devops/stripe-activation/01-site-content-drafts.md`
...
- `components/EndStrip.tsx` — The one footer: brand + tagline, three link columns (Product, Legal: Terms/Refunds/Privacy/Contact, Follow) and the company data line in Italian (required on the site by art. 2250 c.c.). Never links `/pricing`
