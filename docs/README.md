...
- `components/HeroRun.tsx` — One run of competitor-ads in a terminal: tools called with their price, one call held by the Marketing policy and approved, the Ametyst Agent note. Lines fade in with CSS delays, Replay restarts them. `components/TaskFlow.tsx` (the old four-frame demo) is no longer rendered
...
- `components/Faq.tsx` — Six questions, one-sentence answers, native `details`, the `+` right before the question
