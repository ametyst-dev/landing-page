...
> "Step N has no changes for this repo. Should I send STEP_DONE to orchestrator and move on?"
...
```
[DATETIME] [STEP_START] to: orchestrator — starting macro step N on branch {SPRINT_BRANCH}/step-{N}: {description}
```
...
```
[DATETIME] [STEP_DONE] to: orchestrator — macro step N: no changes required, step skipped.
```
